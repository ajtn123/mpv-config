...
local fps_vf_label = '@Danmaku-FPS'
...
local function vf_exists(label)
	local vfs = mp.get_property_native('vf')
	if not vfs then return false end

	for _, vf in ipairs(vfs) do
		if vf.label == label:sub(2) then return true end
	end
	return false
end

-- log to terminal and optionally MPV OSD (On-Screen Display)
...
-- call Danmu2Ass executable to convert danmaku file
function Danmaku_process(cid)
	...
	mp.command_native_async({
		...
		if err == nil and val.status == 0 then
			danmu_file = utils.join_path(danmaku_dir, 'bilibili.ass')
			Danmaku_load(danmu_file)
		else
			log('处理错误: ' .. (err or val.stderr or ('exit code ' .. val.status)))
		end
	end)
end

-- load danmaku into mpv
function Danmaku_load(file)
	...
	local file_info = utils.file_info(file)
	local approximatedDanmukuCount = math.floor((file_info.size - 850) / 120)
	log(file .. ' [' .. file_info.size .. '][' .. approximatedDanmukuCount .. ']')
...
function add_fps_vf()
	...
	if video_fps * video_speed < 45 then
		mp.commandv('vf', 'append', fps_vf_label .. ':lavfi="fps=fps=60:round=down"')
	else
		remove_fps_vf()
	end
end

function remove_fps_vf()
	if vf_exists(fps_vf_label) then
		mp.commandv('vf', 'remove', fps_vf_label)
	end
end

mp.register_event('file-loaded', Danmaku_check)
mp.register_event('end-file', Danmaku_remove)
